...
# Optimization script for spread multiplier thresholds and multipliers in KELP.py
DIR=$(dirname "$0")
RESULTS_DIR="$DIR/../optimization_results_spread_mult"
mkdir -p "$RESULTS_DIR"

TIMESTAMP=$(date +"%Y-%m-%d_%H-%M-%S")
RESULTS_FILE="$RESULTS_DIR/optimization_kelp_spread_mult_${TIMESTAMP}.csv"
...
run_backtest() {
    ...
    local mult_high=$3
    local mult_low=$4
    local tmp_name="KELP_spreadmult_${RANDOM}"
    local base_dir="$(cd "$DIR/../.." && pwd)"
    local strategies_dir="$base_dir/strategies"
    local tmp_file="$strategies_dir/${tmp_name}.py"

    if [ ! -f "$strategies_dir/KELP.py" ]; then
        echo "Error: Source strategy file not found at $strategies_dir/KELP.py"
        return 1
    fi

    cp "$strategies_dir/KELP.py" "$tmp_file"

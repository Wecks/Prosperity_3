...
# Optimization script for spread multiplier thresholds and multipliers in round5_with_counterparty.py
DIR=$(dirname "$0")
RESULTS_DIR="$DIR/../optimization_results_spread_mult"
mkdir -p "$RESULTS_DIR"

TIMESTAMP=$(date +"%Y-%m-%d_%H-%M-%S")
RESULTS_FILE="$RESULTS_DIR/optimization_spread_mult_${TIMESTAMP}.csv"
...
run_backtest() {
    ...
    local mult_high=$3
    local mult_low=$4
    local tmp_name="round5_with_counterparty_spreadmult_${RANDOM}"
    local base_dir="$(cd "$DIR/../.." && pwd)"
    local strategies_dir="$base_dir/strategies"
    local tmp_file="$strategies_dir/${tmp_name}.py"

    if [ ! -f "$strategies_dir/round5_with_counterparty.py" ]; then
        echo "Error: Source strategy file not found at $strategies_dir/round5_with_counterparty.py"
        return 1
    fi

    cp "$strategies_dir/round5_with_counterparty.py" "$tmp_file"

    # Replace the spread multiplier logic with parameterized values
...
# Example grid search
for upper in 0.4 0.5; do
    for lower in 0.2 0.3; do
        for mult_high in 0.9; do
            for mult_low in 0.6; do

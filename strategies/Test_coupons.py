...
class VOLCANIK_ROCKCouponStrategy(SignalStrategy):
    def __init__(self, symbol: Symbol, limit: int) -> None:
        super().__init__(symbol, limit)
    ...
    def get_signal(self, state: TradingState) -> Signal | None:
        if "VOLCANIK_ROCK" not in state.order_depths or len(state.order_depths["VOLCANIK_ROCK"].buy_orders) == 0 or len(state.order_depths["VOLCANIK_ROCK"].sell_orders) == 0:
            return

        if "VOLCANIK_ROCK_COUPON" not in state.order_depths or len(state.order_depths["VOLCANIK_ROCK_COUPON"].buy_orders) == 0 or len(state.order_depths["VOLCANIK_ROCK_COUPON"].sell_orders) == 0:
            return

        coco = self.get_mid_price(state, "VOLCANIK_ROCK")
        coup = self.get_mid_price(state, "VOLCANIK_ROCK_COUPON")
...
class Trader:
    def __init__(self) -> None:
        ...
        self.strategies: dict[Symbol, Strategy] = {symbol: clazz(symbol, limits[symbol]) for symbol, clazz in {
            ...
            "VOLCANIK_ROCK_COUPON": VOLCANIK_ROCKCouponStrategy,
        }.items()}

...
class Trader:
    ...
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        if len(self.history1) > self.window:
            ...
            if z > 2.3 * self.z_entry and pos1 > -20:
                orders[self.asset1].append(Order(self.asset1, max(d1.buy_orders), -5))
                orders[self.asset2].append(Order(self.asset2, min(d2.sell_orders), 5))
                logger.print("Short CROISSANTS, Long PICNIC_BASKET2")

            elif z < -2.3 * self.z_entry and pos1 < 20:
                orders[self.asset1].append(Order(self.asset1, min(d1.sell_orders), 5))
                orders[self.asset2].append(Order(self.asset2, max(d2.buy_orders), -5))
                logger.print("Long CROISSANTS, Short PICNIC_BASKET2")

            elif abs(z) < 0.4 * self.z_exit:
                if pos1 != 0:

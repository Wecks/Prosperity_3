...
class Trader:
    ...
    def market_make(
        ...
        buy_qty = self.LIMIT[product] - (position + buy_order_volume)
        if buy_qty > 0:
            orders.append(Order(product, round(bid), buy_qty))
        sell_qty = self.LIMIT[product] + (position - sell_order_volume)
        if sell_qty > 0:
    ...
    def clear_position_order(
        ...
        if position_after > 0:
            clear_vol = sum(v for p, v in order_depth.buy_orders.items() if p >= fair_ask)
            sent = min(clear_vol, position_after, sell_qty)
            if sent > 0:
                orders.append(Order(product, fair_ask, -abs(sent)))
                sell_order_volume += abs(sent)

        if position_after < 0:
            clear_vol = sum(abs(v) for p, v in order_depth.sell_orders.items() if p <= fair_bid)
            sent = min(clear_vol, abs(position_after), buy_qty)
            if sent > 0:
    ...
    def make_amethyst_orders(
        ...
        buy_order_volume, sell_order_volume = self.market_make(
            Product.AMETHYSTS, orders, bbbf+1, baaf-1,
    ...
    def make_starfruit_orders(
        ...
        buy_order_volume, sell_order_volume = self.market_make(
            Product.STARFRUIT, orders, bbbf+1, baaf-1,
            position, buy_order_volume, sell_order_volume
        )
        return orders, buy_order_volume, sell_order_volume
    ...
    def MAGNIFICENT_MACARONS_adap_edge(
        ...
        dec_cond = self.params[Product.MAGNIFICENT_MACARONS]["dec_edge_discount"] * \
    ...
    def MAGNIFICENT_MACARONS_arb_take(
        ...
        for price in sorted(order_depth.sell_orders.keys()):
            if price > implied_bid - edge:
                break
            qty = min(abs(order_depth.sell_orders[price]), buy_qty)
            if qty > 0:
        ...
        for price in sorted(order_depth.buy_orders.keys(), reverse=True):
            if price < implied_ask + edge:
                break
            qty = min(abs(order_depth.buy_orders[price]), sell_qty)
            if qty > 0:
    ...
    def run(self, state: TradingState):
        ...
        if (Product.MAGNIFICENT_MACARONS in state.order_depths
            ...
            conversions = 0  # pas de clear conversion
            ...
            orders_take, buy_volume, sell_volume = self.MAGNIFICENT_MACARONS_arb_take(od, obs, edge, pos)
            orders_make, _, _ = self.MAGNIFICENT_MACARONS_arb_make(od, obs, pos, edge, buy_volume, sell_volume)

...
class Trader:
    ...
    def MAGNIFICENT_MACARONS_adap_edge(
        ...
        if len(self.MAGNIFICENT_MACARONS_data["volume_history"]) < self.params[Product.MAGNIFICENT_MACARONS]["volume_avg_timestamp"]:
            ...
            dec_cond = self.params[Product.MAGNIFICENT_MACARONS]["dec_edge_discount"] * self.params[Product.MAGNIFICENT_MACARONS]["volume_bar"] * (curr_edge - self.params[Product.MAGNIFICENT_MACARONS]["step_size"])
    ...
    def MAGNIFICENT_MACARONS_arb_make(
        ...
        print(f"FOREIGN_MID: {foreign_mid}")
        print(f"ALGO_BID: {bid}")
        print(f"ALGO_ASK: {ask}")

        buy_quantity = position_limit - (position + buy_order_volume)
        if buy_quantity > 0:
            orders.append(Order(Product.MAGNIFICENT_MACARONS, aggressive_ask - 1, buy_quantity))

        # sell logic commented out
        return orders, buy_order_volume, sell_order_volume
    ...
    def run(self, state: TradingState):
        ...
        if (Product.MAGNIFICENT_MACARONS in state.order_depths
            ...
            print(f"MAGNIFICENT_MACARONS_EDGE: {edge}")
